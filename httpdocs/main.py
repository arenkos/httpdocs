...
def process_pdf_content(pdf_content, derece):
    ...
    def remove_duplicate_course_codes(lines):
        unique_lines = []
        seen_codes = set()  # Görülen ders kodlarını izlemek için bir küme oluştur
        for line in lines:
            code = ' '.join(line.split(None, 2)[:2])  # İlk 2 kelimeyi ders kodu olarak al
            if code not in seen_codes:  # Eğer ders kodu daha önce görülmediyse
                unique_lines.append(line)  # Satırı sonuç listesine ekle
                seen_codes.add(code)  # Ders kodunu görülenler listesine ekle
        return unique_lines
