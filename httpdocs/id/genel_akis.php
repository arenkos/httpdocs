...
<head>
	...
	<title>AR Haber - Genel Akış</title>
	<!-- Google tag (gtag.js) -->
...
<body style="margin:0 auto;background-color:#222222;width:100%;margin-top:-5px;">
	<div style="position:fixed;height:80px;width:100%;margin:0 auto;text-align:center;background-color:black;">
		<form  method="GET" action="genel_akis.php">
	...
	<div style="height:auto;width:100vw;max-width:60vh;margin:5 auto;">
		...
		<?php
			...
			if(isset($_GET['arama'])){
				$arama = $_GET['arama'];
				if(!empty($row['kategori']) && !empty($row['kategori'])){
					$sql = "SELECT * FROM haberler WHERE (baslik LIKE '%$arama%' OR kaynak LIKE '%$arama%' OR kategori LIKE '%$arama%') AND resim_url NOT LIKE '' AND kaynak LIKE '$kaynak_ozellestirme' AND kategori LIKE '$kategori_ozellestirme' ORDER BY tarih DESC";
				}else if(!empty($row['kategori'])){
					$sql = "SELECT * FROM haberler WHERE (baslik LIKE '%$arama%' OR kaynak LIKE '%$arama%' OR kategori LIKE '%$arama%') AND resim_url NOT LIKE '' AND kategori LIKE '$kategori_ozellestirme' ORDER BY tarih DESC";
				}else if(!empty($row['kaynak'])){
					$sql = "SELECT * FROM haberler WHERE (baslik LIKE '%$arama%' OR kaynak LIKE '%$arama%' OR kategori LIKE '%$arama%') AND resim_url NOT LIKE '' AND kaynak LIKE '$kaynak_ozellestirme' ORDER BY tarih DESC";
				}else{
				...
				if(!empty($row['kategori']) && !empty($row['kategori'])){
					$sql = "SELECT * FROM haberler WHERE resim_url NOT LIKE '' AND kaynak LIKE '$kaynak_ozellestirme' AND kategori LIKE '$kategori_ozellestirme' ORDER BY tarih DESC";
				}else if(!empty($row['kategori'])){
					$sql = "SELECT * FROM haberler WHERE resim_url NOT LIKE '' AND kategori LIKE '$kategori_ozellestirme' ORDER BY tarih DESC";
				}else if(!empty($row['kaynak'])){
					$sql = "SELECT * FROM haberler WHERE resim_url NOT LIKE '' AND kaynak LIKE '$kaynak_ozellestirme' ORDER BY tarih DESC";
				}else{
