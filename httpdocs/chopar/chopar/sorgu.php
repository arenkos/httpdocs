...
<?php include 'teklif_formu.php'; ?>
    ...
    if ($result->num_rows > 0) {
        ...
        <div style="width:100%;text-algn:center;">
...
<?php } ?>
    ...
    <div style="width:100%;height: auto;margin-top:30px;">
        <table style="width:60%;height: auto;margin:0 auto;">
            ...
            <?php
            $gelenDeger = $_POST['siparis_no']; // Kullanıcıdan alınan değeri burada varsayalım

            $sip = intval(substr($gelenDeger, 3));
            $sql = "SELECT * FROM siparis WHERE siparis_no='$sip' AND adet!=0";
            mysqli_set_charset($connection, "utf8");
            $result = mysqli_query($connection, $sql);
            $t_fiyat = floatval(0);
            $i = 1;
            if($result->num_rows > 0){
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    <tr style="width: 100%;height: 200px;margin: 0 auto;padding:10px;">
                        ...
                        <td style="width:10%;height=100%;text-align:ceter;">
                            <p style="color:white;"><?php echo $row['kod']; ?></p>
                        </td>
                        <td style="width:20%;height=100%;text-align:ceter;">
                            <p style="color:white;">
                                <?php
                        ...
                        <td style="width:10%;height=100%;text-align:ceter;">
                            <p style="color:white;"><?php echo $row['toplam_fiyat']; ?> TL</p>
                        </td>
                    </tr>
                <?php
                    $i = $i + 1;
                    $t_fiyat += floatval($row['toplam_fiyat']);
                }
            }
            ?>
            <tr style="width: 100%;height: 100px;margin: 0 auto;padding:10px;">
